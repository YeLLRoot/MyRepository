...
namespace DWS.Class
{
    class AutoSizeForm
    {
        ...
        private void GetControlSize(Control con)
        {
            ...
            foreach (Control c in con.Controls)
            {
                ...
                if (c.GetType().ToString() == "System.Windows.Forms.Panel"
                    || c.GetType().ToString() == "System.Windows.Forms.GroupBox"
                    || c.GetType().ToString() == "System.Windows.Forms.TabControl"
                    || c.GetType().ToString() == "System.Windows.Forms.TabPage"
                    )
                {
                    GetControlSize(c);
                }
        ...
        public void controlAutoSize(Form mForm)
        {
            ...
            SetSize(mForm, ref ctrlNo, wScale, hScale);

        }

        private void SetSize(Control con,ref int ctrlNo, float wScale, float hScale)
        {
            ...
            foreach (Control c in con.Controls)
            {
                ...
                c.Font = new Font(c.Font.Name, (float)(ctrFontSize0 * wScale));
                ...
                {
                    SetSize(c, ref ctrlNo, wScale, hScale);//设置控件容器中的控件大小。  
